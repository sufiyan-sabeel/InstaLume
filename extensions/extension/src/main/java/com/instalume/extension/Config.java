...
/**
 * InstaLume runtime configuration backed by SharedPreferences ("instalume_prefs").
 *
 * Independent block toggles (feed, explore, reels, stories, instants, notes,
 * suggested, ads) plus the permanent "hardcore" lock, theme/navigation/privacy
 * keys, and developer keys. The application Context
 * is resolved reflectively so no patched class needs to hand us one.
 *
 * Migration: V1.0.0 renames the store from FeurStagram's "feurstagram_prefs".
 * On first access, existing keys are copied once, then the legacy store is left
 * untouched (never deleted, to allow downgrade inspection).
 */
public final class Config {

    private static final String PREFS = "instalume_prefs";
    private static final String LEGACY_PREFS = "feurstagram_prefs";
    ...
    private static synchronized void maybeMigrate(Context context) {
        if (sMigrated) return;
        sMigrated = true;
        try {
            SharedPreferences cur = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE);
            if (cur != null && !cur.getAll().isEmpty()) return;
            SharedPreferences legacy = context.getSharedPreferences(LEGACY_PREFS, Context.MODE_PRIVATE);
            if (legacy == null || legacy.getAll().isEmpty()) return;
    ...
    /** Whether to check GitHub for a newer release on launch. OFF by default in InstaLume (explicit consent). */
    public static boolean isAutoUpdateEnabled() {
        return getBlocked("auto_update", false);
    }

    // --- InstaLume V1.0.0: themes / glass / nav / privacy / misc / developer ---

    public static String getThemeMode() {
        SharedPreferences p = prefs();
        String v = p == null ? "system" : p.getString("theme_mode", "system");
        return ("light".equals(v) || "dark".equals(v)) ? v : "system";
    }
    public static void setThemeMode(String v) {
        if (!("light".equals(v) || "dark".equals(v) || "system".equals(v))) return;
        SharedPreferences p = prefs();
        if (p == null) return;
        p.edit().putString("theme_mode", v).apply();
    }

    public static boolean isGlassEnabled() { return getBlocked("glass_enabled", true); }
    public static String getGlassMode() {
        SharedPreferences p = prefs();
        String v = p == null ? "ios" : p.getString("glass_mode", "ios");
        return "universal".equals(v) ? "universal" : "ios";
    }
    public static void setGlassMode(String v) {
        SharedPreferences p = prefs();
        if (p == null) return;
        p.edit().putString("glass_mode", "universal".equals(v) ? "universal" : "ios").apply();
    }
    public static int getGlassOpacity() { return intClamped("glass_opacity", 72, 20, 95); }
    public static int getBlurStrength() { return intClamped("blur_strength", 18, 0, 40); }
    public static int getCornerRadius() { return intClamped("corner_radius", 24, 0, 32); }
    public static int getNavSize() { return intClamped("nav_size", 100, 80, 120); }
    public static int getNavYOffset() { return intClamped("nav_y_offset", 0, -24, 24); }
    ...
    public static boolean isReducedMotion() { return getBlocked("reduced_motion", false); }
    public static boolean isCompactUi() { return getBlocked("compact_ui", false); }
    public static boolean isAutoCacheClean() { return getBlocked("auto_cache_clean", false); }
    public static boolean isUrlSanitize() { return getBlocked("url_sanitize", true); }
    public static boolean isDistractionFree() { return getBlocked("distraction_free", false); }
    public static boolean isDebugLogging() { return getBlocked("debug_logging", false); }
